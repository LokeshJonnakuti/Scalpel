...
for i, c in enumerate(coordinates):
    folium.CircleMarker(
        [c[0], c[1]],
        radius=most_common_locations[places_list[i]] * 0.25,
        color="#3186cc",
        fill=True,
        fill_opacity=0.5,
        fill_color="#3186cc",
        popup="{} ({}, {}) appears {} times in book.".format(
            places_list[i], c[0], c[1], most_common_locations[places_list[i]]
        ),
    ).add_to(basemap)
print(
    "Map of relevant locations in Broneer et al.'s \"Ancient Corinth: A guide to the"
    ' excavations," weighted by frequency.'
)
basemap
page = 87
...
for i in range(1, max(counts["page"]) + 1):
    ...
    tokens = [
        token for token in tokens if token.lower() not in stops and len(token) > 2
    ]
...
for i, c in enumerate(coordinates):
    folium.CircleMarker(
        [c[0], c[1]], color="#3186cc", fill=True, fill_opacity=0.5, fill_color="#3186cc"
    ).add_to(basemap)
basemap
