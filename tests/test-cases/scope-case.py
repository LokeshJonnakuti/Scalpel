...
r = requests.get(
    "http://3d-kenya.chordsrt.com/instruments/1.geojson?start=2016-09-01T00:00&end=2016-11-01T00:00"
)
if r.status_code == 200:
    d = r.json()["Data"]
else:
    print(
        "Please verify that the URL for the weather station is correct. You may just"
        " have to try again with a different/smaller date range or different dates."
    )
# In[25]:
davad_tuple = (
    "f1",
...
def make_data_set(d):
    ...
    for o in d:
        if o["variable_shortname"] == "rain":
            ...
            if (
                ttime.endswith("00")
                or ttime.endswith("15")
                or ttime.endswith("30")
                or ttime.endswith("45")
            ):
