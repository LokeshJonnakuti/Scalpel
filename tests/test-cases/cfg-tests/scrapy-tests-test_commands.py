...
from os.path import abspath, exists, getmtime, join
...
class StartprojectTemplatesTest(ProjectTest):
    ...
    def test_startproject_template_override(self):
        ...
        self.assertIn(
            f"New Scrapy project '{self.project_name}', using template directory", out
        )
...
class GenspiderCommandTest(CommandTest):
    ...
    def test_template(self, tplname="crawl"):
        ...
        self.assertIn(
            (
                f"Created spider {spname!r} using template {tplname!r} in"
                f" module:{os.linesep}  {spmodule}"
            ),
            out,
        )
...
"""
        ...
        self.assertIn(
            (
                '[myspider] DEBUG: FEEDS: {"example.json": {"format": "json",'
                ' "overwrite": true}}'
            ),
            log,
        )
