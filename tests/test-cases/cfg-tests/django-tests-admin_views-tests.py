...
ERROR_MESSAGE = (
    "Please enter the correct username and password for a staff account. Note that both"
...
class AdminViewPermissionsTest(TestCase):
    ...
    def test_add_view(self):
        ...
        self.assertNotContains(
            ...
            change_list_link,
            msg_prefix=(
                "User restricted to add permission is given link to change list view in"
                " breadcrumbs."
            ),
        )
        post = self.client.post(reverse("admin:admin_views_article_add"), add_dict)
    ...
    def test_post_save_message_no_forbidden_links_visible(self):
        ...
        self.assertContains(
            response,
            (
                '<li class="success">The article “Fun &amp; games” was added'
                " successfully.</li>"
            ),
            html=True,
        )
...
class AdminViewListEditable(TestCase):
    ...
    def test_pk_hidden_fields(self):
        ...
        self.assertContains(
            ...
            ' value="%d" id="id_form-0-id"><input type="hidden" name="form-1-id"'
            ' value="%d" id="id_form-1-id">\n</div>' % (story2.id, story1.id),
            html=True,
        )
...
class ReadonlyTest(AdminFieldExtractionMixin, TestCase):
    ...
    def test_readonly_get(self):
        ...
        self.assertContains(
            response,
            (
                '<div class="help">Some help text for the title (with Unicode'
                " ŠĐĆŽćžšđ)</div>"
            ),
            html=True,
        )
        self.assertContains(
            response,
            (
                '<div class="help">Some help text for the content (with Unicode'
                " ŠĐĆŽćžšđ)</div>"
            ),
            html=True,
        )
        self.assertContains(
            response,
            (
                '<div class="help">Some help text for the date (with Unicode'
                " ŠĐĆŽćžšđ)</div>"
            ),
            html=True,
        )
...
class AdminKeepChangeListFiltersTests(TestCase):
    ...
    def test_assert_url_equal(self):
        ...
        self.assertURLEqual(
            "http://testserver{}?_changelist_filters=is_staff__exact%3D0%26is_superuser__exact%3D0"
            .format(change_user_url),
            "http://testserver{}?_changelist_filters=is_staff__exact%3D0%26is_superuser__exact%3D0"
            .format(change_user_url),
        )
        ...
        with self.assertRaises(AssertionError):
            self.assertURLEqual(
                "http://testserver{}?_changelist_filters=is_staff__exact%3D0%26is_superuser__exact%3D0"
                .format(change_user_url),
                "http://testserver{}?_changelist_filters=is_staff__exact%3D1%26is_superuser__exact%3D1"
                .format(change_user_url),
            )
        ...
        self.assertURLEqual(
            "http://testserver{}?_changelist_filters=is_staff__exact%3D0%26is_superuser__exact%3D0"
            .format(change_user_url),
            "{}?_changelist_filters=is_staff__exact%3D0%26is_superuser__exact%3D0"
            .format(change_user_url),
        )
        ...
        self.assertURLEqual(
            "{}?_changelist_filters=is_staff__exact%3D0%26is_superuser__exact%3D0"
            .format(change_user_url),
            "{}?_changelist_filters=is_superuser__exact%3D0%26is_staff__exact%3D0"
            .format(change_user_url),
        )
...
class AdminViewOnSiteTests(TestCase):
    ...
    def test_add_view_form_and_formsets_run_validation(self):
        ...
        self.assertFormsetError(
            ...
            None,
            [
                "Children must share a family name with their parents in this contrived"
                " test case"
            ],
        )
        msg = (
            "The formset 'inline_admin_formset' in context 12 does not contain any"
            " non-form errors."
        )
        with self.assertRaisesMessage(AssertionError, msg):
            self.assertFormsetError(
        ...
        try:
            ...
            self.assertEqual(
                ...
                [
                    Error(
                        (
                            "The value of 'view_on_site' must be a callable or a"
                            " boolean value."
                        ),
                        obj=CityAdmin,
                        id="admin.E025",
