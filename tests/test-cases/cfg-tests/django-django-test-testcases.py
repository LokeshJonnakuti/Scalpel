    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if not self.test():
            ...
            if self.rendered_templates:
                message += " Following templates were rendered: %s" % ", ".join(
                    self.rendered_template_names
                )
            else:
        ...
        if hasattr(response, "redirect_chain"):
            ...
            if fetch_redirect_response:
                ...
                if domain and not validate_host(domain, settings.ALLOWED_HOSTS):
                    raise ValueError(
                        "The test client is unable to fetch remote URLs (got %s). If"
                        " the host is served by Django, add '%s' to ALLOWED_HOSTS."
            ...
            for err in errors:
                if field:
                    if field in context[form].errors:
                        ...
                        self.fail(
                            msg_prefix
                            + "The form '%s' in context %d does not contain the field"
                            " '%s'" % (form, i, field)
                        )
                else:
            ...
            for err in errors:
                if field is not None:
                    if field in context[formset].forms[form_index].errors:
                        ...
                        self.fail(
                            msg_prefix
                            + "The field '%s' on formset '%s', form %d in context %d"
                            " contains no errors" % (field, formset, form_index, i)
                        )
                    else:
                        self.fail(
                            msg_prefix
                            + "The formset '%s', form %d in context %d does not contain"
                            " the field '%s'" % (formset, form_index, i, field)
                        )
