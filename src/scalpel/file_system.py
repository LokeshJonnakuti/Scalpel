...
def format_import_path(import_stmt, abs_path):
    ...
    if isinstance(import_stmt, ast.Import):
        ...
        abs_import_path + [
            import_path_parts[: -import_stmt.level] + "." + import_stmt.module
        ]
        pass
    else:
...
class Node:
    ...
    def __init__(
        self, name, abs_path, mod_src, mod_ast, parent=None, node_type="module"
    ):
        ...
        self.node_type = (
            node_type  #   there are two different node:  module and package
        )
        self.parent = parent
